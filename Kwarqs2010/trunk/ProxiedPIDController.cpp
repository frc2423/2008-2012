	...
	m_semaphore (0)
{
	...
	m_dma_input = webdma.CreateFloatProxy(groupName, "Input", FloatProxyFlags().default_value(0).readonly());
	m_dma_result = webdma.CreateFloatProxy(groupName, "Result", FloatProxyFlags().default_value(0).readonly());
	m_dma_error = webdma.CreateFloatProxy(groupName, "Error", FloatProxyFlags().default_value(0).readonly());
	m_dma_totalError = webdma.CreateDoubleProxy(groupName, "Tot Error", DoubleProxyFlags().default_value(0).readonly());
...
void ProxiedPIDController::Calculate()
{
	...
	if (m_enabled)
	{
		...
		// PID display values
		m_dma_result = m_result;
		m_dma_error  = m_error;
		m_dma_totalError = m_totalError;
	}
	}
	END_REGION;
}
