	...
	m_semaphore (0)
{
	...
	mp_P = WebInterface::CreateFloatProxy(groupName, "P", FloatProxyFlags().default_value(Kp).minval(0).maxval(1).step(0.01F));
	mp_I = WebInterface::CreateFloatProxy(groupName, "I", FloatProxyFlags().default_value(Ki).minval(0).maxval(1).step(0.01F));
	mp_D = WebInterface::CreateFloatProxy(groupName, "D", FloatProxyFlags().default_value(Kd).minval(0).maxval(1).step(0.01F));
	m_maximumOutput = 1.0;
	m_minimumOutput = -1.0;
